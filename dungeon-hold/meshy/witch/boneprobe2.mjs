...
await new Promise(r=>server.listen(8877,"127.0.0.1",r)); const browser=await chromium.launch({args:["--use-gl=angle","--use-angle=swiftshader","--enable-unsafe-swiftshader"]}); const page=await browser.newPage();
await page.goto("http://127.0.0.1:8877/boneprobe2.html"); await page.waitForFunction(()=>window.probe); const r=await page.evaluate(()=>window.probe()); console.log(JSON.stringify(r)); await browser.close(); server.close();
