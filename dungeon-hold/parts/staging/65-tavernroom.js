...
const at=(x,z)=>[cw(x),cwz(z)];   // grid → world
...
// --- stations ---
const STATIONS=[{x:lx+.6,z:lz,tab:'bag',what:'your locker (bag)'},{x:bx-1.0,z:bz,tab:'shop',what:'the barkeep (buy & sell)'},{x:tx+.4,z:tz-.4,tab:'skills',what:'the trainer (skills)'},{x:window.__anvil[0]-1.0,z:window.__anvil[1]+.3,tab:'forge',what:'the anvil (upgrade your gear)'}];
...
function visit(st){ if(st.tab==='forge'){ if(window.__doll&&window.__doll.open(true)){ SFX.place(); return true; } return false; } if(!(typeof Tavern!=='undefined'&&Tavern)) return false; Tavern.open(); Tavern.tab(st.tab); SFX.place(); return true; }
...
window.__room={stations:()=>STATIONS.map(s=>({x:+s.x.toFixed(2),z:+s.z.toFixed(2),tab:s.tab})),near:()=>near&&near.tab,door:[dx,dz],bounds:{x0:cw(12)-1,x1:cw(20)+1,z0:cwz(25)-1,z1:cwz(31)+1}};
})();
