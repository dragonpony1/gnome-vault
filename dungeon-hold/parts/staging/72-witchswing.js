// ===== THE WITCH'S STRIKE: her Meshy "whip attack" clip is a high kick, so her swing is made by hand instead — the whip arm
// winds up over her shoulder and snaps forward while the body keeps its idle or walk, and the rope lash does the rest. Heroes
// opt in by id in PROC (which arm); the clip stays in the file but never plays for them, and the hit lands at the snap.
(function(){
const PROC={witch:'Left'};
...
window.__armSwing={side,bones:()=>{ const B=bones(); return B?{up:B.up.name,fore:B.fore.name,hand:B.hand.name}:null; }};
})();
