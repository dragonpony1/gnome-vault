...
const Tavern={
  ...
  summary(data){ tvBuildDom(); TV.sumData=data; TV.open=true; TV.sum=true; TV.sel=null; TV.lastT=performance.now(); tvClearMsg(); tvShield(true); if(document.pointerLockElement&&document.exitPointerLock) document.exitPointerLock(); $('dead').classList.add('hide'); $('banner').style.opacity=0;
    ...
    $('tv-sum').innerHTML='<h1>SHATTERED</h1><h2>THE CRYSTAL FELL ON WAVE '+(data.wave|0)+(data.newBest?' · A NEW BEST':'')+'</h2><div class="tv-stats">'+stat(tvG(data.kills||0),'KILLS')+stat('+'+tvG(data.xpGained||0),'XP')+stat('+'+tvG(data.goldGained||0)+' ●','GOLD EARNED'+(spent?'<small>−'+tvG(spent)+' spent</small>':''))+stat('+'+(data.levelsGained||0),'LEVELS')+'</div>'+dh
     +'<p style="color:#c9b8a0;font-size:13px;margin:0 0 6px">Your gear, gold and skills stay with you.</p><div class="db"><button class="tv-btn hot" data-act="totavern" id="tv-totavern">🍺 TO THE TAVERN</button><button class="tv-btn" data-act="again" id="tv-again">↻ GO AGAIN</button></div>';
