...
const DEFS={
  ...
  totem:{name:'Frost Totem',ic:'❄',du:4,mana:70,hp:120,top:2.8,range:7,rangeUp:1,arc:360,buff:.15,buffUp:.05,chill:.7}};    // a runed pillar: defenses in its ring hit 15% harder and faster (+5% a mark), mobs in it crawl at 70%; totems never stack                                       // a thorn wall that hurts attackers and regrows when left alone
const DEFKEYS=['harpoon','acorn','ball','slice','spike','totem']; const MAXLVL=5, MARK=['','I','II','III','IV','V'];
...
function mobSpd(e){ return e.spd*(e.slowT>0?DEFS.slice.slow:1)*(e.chillT>0?DEFS.totem.chill:1); }   // spored mobs crawl; chilled ones too
...
function updateEnemies(dt){
  ...
  for(let i=enemies.length-1;i>=0;i--){ const e=enemies[i]; const g=e.mdl.g;
    ...
    e.pop=Math.min(1,e.pop+dt*3); e.atk-=dt; e.slowT=Math.max(0,(e.slowT||0)-dt); e.chillT=Math.max(0,(e.chillT||0)-dt); if(e.swing>=0){ e.swing+=dt; if(e.pending&&e.swing>=.2){ const tg=e.pending; e.pending=null; landHit(e,tg); } if(e.swing>.4) e.swing=-1; }
...
function stat(d,k){ const cfg=DEFS[d.kind], l=d.lvl||1; if(k==='dmg') return Math.max(1,Math.round(cfg.dmg*(1+.5*(l-1))*(1+heroStat('tow')/100)*heroMult('tow')*(1+(d.buff||0))*10)/10); if(k==='cd') return cfg.cd*Math.pow(.8,l-1)/heroMult('tcd')/(1+heroStat('trate')/100)/(1+(d.buff||0)); if(k==='buff') return (cfg.buff||0)+(cfg.buffUp||0)*(l-1); if(k==='range') return ((cfg.range||0)+(cfg.rangeUp!==undefined?cfg.rangeUp:2)*(l-1))*(cfg.arc===360?heroMult('aoe'):1)*(1+heroStat('tarea')/100); return cfg[k]; }
...
function updateDefs(dt){ const trampled=[];
  ...
  for(const d of defs){ const cfg=DEFS[d.kind]; d.pop=Math.min(1,d.pop+dt*4); const s=(d.pop<1?easeOutBack(d.pop):1)*(d.kind==='slice'?stat(d,'range')/cfg.range:(1+.07*(d.lvl-1))); d.mdl.scale.set(s,s,s); d.cd-=dt; d.shake=Math.max(0,d.shake-dt); d.recoil=Math.max(0,d.recoil-dt*4);
    ...
    else if(d.kind==='totem'){ const rr=stat(d,'range'); let n=0; for(const e of enemies){ if(!e.dead&&Math.hypot(e.x-d.x,e.z-d.z)<rr+e.r*.5){ e.chillT=.5; n++; } }
      let a=d.mdl.userData.aura; if(!a){ a=new THREE.Group(); const ring=new THREE.Mesh(new THREE.RingGeometry(.94,1,48),new THREE.MeshBasicMaterial({color:0x5ec8ff,transparent:true,opacity:.35,side:THREE.DoubleSide,depthWrite:false,blending:THREE.AdditiveBlending})); ring.rotation.x=-PI/2; ring.userData.noOL=true; a.add(ring); const inner=new THREE.Mesh(new THREE.RingGeometry(.2,.24,24),ring.material.clone()); inner.rotation.x=-PI/2; inner.userData.noOL=true; a.add(inner); const plume=glow(0x5ec8ff,1.5,.55); a.add(plume); a.userData.ring=ring; a.userData.inner=inner; a.userData.plume=plume; d.mdl.add(a); d.mdl.userData.aura=a; }   /* the ring on the floor at the buff's reach, a small spinner, the plume at the top */
      a.position.y=.03; a.scale.set(rr/s,1/s,rr/s); a.userData.plume.scale.set(1.5/rr,1.5,1); a.userData.plume.position.set(0,cfg.top-.1,0);   /* the group undoes the model's own scale: ring radius in world units, heights untouched */ a.userData.inner.rotation.z+=dt*(n?2.5:.8); a.userData.ring.material.opacity=.28+.1*Math.sin(S.t*2.4)+(n?.12:0); a.userData.plume.material.opacity=.45+.15*Math.sin(S.t*3.1); } }
...
addEventListener('keydown',e=>{ const c=e.code; if(Meta.isOpen()) return; if(c==='KeyI'||c==='KeyB'){ if(!e.repeat) Meta.open(); return; } if(S.phase==='start'){ if(c==='Enter'||c==='Space'){ e.preventDefault(); play(); } return; }
  ...
  if(c==='Digit1') select('harpoon'); if(c==='Digit2') select('acorn'); if(c==='Digit3') select('ball'); if(c==='Digit4') select('slice'); if(c==='Digit5') select('spike'); if(c==='Digit6') select('totem');
