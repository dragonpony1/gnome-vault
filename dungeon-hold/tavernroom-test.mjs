...
await page.goto("http://127.0.0.1:8854/?silent"); await page.waitForFunction(()=>window.__dd&&window.__dd.heroModel&&window.__dd.heroModel()&&window.__dd.mobModel("goblin")&&window.__room,null,{timeout:60000});
...
const r1=await page.evaluate(async()=>{ const d=window.__dd, T=window.__tavern; const out=[]; for(const st of window.__room.stations()){ d.setHero(st.x-.8,st.z,0); d.step(1/60,3); const prompt=document.getElementById("prompt").textContent; window.dispatchEvent(new KeyboardEvent("keydown",{code:"KeyE",bubbles:true})); await new Promise(r=>setTimeout(r,60)); const forge=st.tab==="forge"; const open=forge?window.__doll.isOpen():T.isOpen(), tab=forge?"forge":(T.state?T.state().tab:document.querySelector("#tavern .tv-tab.on,#tavern [data-tab].on,#tavern .on")?.textContent); window.dispatchEvent(new KeyboardEvent("keydown",{code:"Escape",bubbles:true})); await new Promise(r=>setTimeout(r,60)); out.push({near:window.__room.near(),prompt,open,tab,closed:!(forge?window.__doll.isOpen():T.isOpen())}); } return out; });
check("locker → bag, barkeep → shop, trainer → skills, anvil → forge (prompt + E + Escape)",r1.length===4&&r1.every(o=>o.open&&o.closed&&/^E /.test(o.prompt))&&r1[0].near==="bag"&&r1[1].near==="shop"&&r1[2].near==="skills"&&r1[3].near==="forge"&&String(r1[0].tab).toLowerCase().includes("bag")&&String(r1[1].tab).toLowerCase().includes("shop")&&String(r1[2].tab).toLowerCase().includes("skill")&&/anvil/.test(r1[3].prompt),JSON.stringify(r1));
