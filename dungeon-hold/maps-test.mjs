...
await page.goto("http://127.0.0.1:8881/?silent"); await ready(); await page.evaluate(()=>window.__campaign.unlockAll());
const run=async(mapi)=>{ await page.goto("http://127.0.0.1:8881/?silent&map="+mapi); await ready(); return page.evaluate(()=>{ const d=window.__dd; window.__meta.reset(); d.resetGear(); d.start(); d.step(1/60,5); const L=d.lanes(); const paths=Object.fromEntries(Object.keys(L).map(k=>[k,d.pathLen(L[k].cx,L[k].cz)])); const m=d.map();
...
check("map 4 The Great Feast Hall: long hall with windows, three doors with paths ≥ 20 (east ≥ 38), crystal on the high-table dais a step up, goblins thread the tables to it",f.id==="feast"&&f.windows>0&&f.lanes.join()==="E,N,S"&&f.paths.E>=38&&Object.values(f.paths).every(p=>p>=20)&&f.crystalH===1&&f.heroY>=.9&&Object.values(f.reach).every(r=>r.ok)&&f.stations===4&&/MAP 4 OF 4/.test(f.line),JSON.stringify(f));
...
await page.goto("http://127.0.0.1:8881/?silent&map=2"); await ready();
